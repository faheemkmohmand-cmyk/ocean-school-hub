...
// We use a GeoJSON world layer from a public CDN and match clicked features by name
...
const WeekTab = ({
  ...
  const handleBorder = (cca3: string) => {
    // no-op in week tab — could open map
  };
...
const CompareTab = ({ countries }: { countries: Country[] }) => {
  ...
  const makeSearch = (
    set: (v: string) => void,
    setSugs: (v: Country[]) => void
  ) => (q: string) => {
    set(q);
    if (q.length < 2) { setSugs([]); return; }
    const lq = q.toLowerCase();
    setSugs(countries.filter((c) => c.name.common.toLowerCase().includes(lq)).slice(0, 5));
  };
    ...
    <div className="space-y-4">
      <div className="flex gap-3">
        <Picker
          ...
          search={searchA}
          sugs={sugsA}
          onSearch={makeSearch(setSearchA, setSugsA)}
          onPick={(c) => pick(c, setA, setSearchA, setSugsA)}
          country={a}
        />
        ...
        <Picker
          ...
          search={searchB}
          sugs={sugsB}
          onSearch={makeSearch(setSearchB, setSugsB)}
          onPick={(c) => pick(c, setB, setSearchB, setSugsB)}
          country={b}
        />
      </div>
...
export default function WorldExplorer() {
  ...
  useEffect(() => {
    // REST Countries API now limits requests to a maximum of 10 fields per call.
    // We split the 20 required fields across 3 requests and merge by cca2.
    const batch1 = "cca2,cca3,name,capital,population,area,region,subregion,languages,currencies";
    const batch2 = "cca2,borders,flags,coatOfArms,latlng,timezones,continents,fifa,tld,independent";
    const batch3 = "cca2,unMember";

    const BASE = "https://restcountries.com/v3.1/all?fields=";

    Promise.all([
      fetch(`${BASE}${batch1}`).then((r) => { if (!r.ok) throw new Error("API error"); return r.json(); }),
      fetch(`${BASE}${batch2}`).then((r) => { if (!r.ok) throw new Error("API error"); return r.json(); }),
      fetch(`${BASE}${batch3}`).then((r) => { if (!r.ok) throw new Error("API error"); return r.json(); }),
    ])
      .then(([d1, d2, d3]: [Partial<Country>[], Partial<Country>[], Partial<Country>[]]) => {
        // Build lookup maps keyed by cca2
        const map2 = new Map<string, Partial<Country>>();
        const map3 = new Map<string, Partial<Country>>();
        d2.forEach((c) => { if (c.cca2) map2.set(c.cca2, c); });
        d3.forEach((c) => { if (c.cca2) map3.set(c.cca2, c); });

        // Merge all batches into full Country objects
        const merged: Country[] = d1.map((c) => {
          const code = c.cca2 ?? "";
          const b2 = map2.get(code) ?? {};
          const b3 = map3.get(code) ?? {};
          return { ...c, ...b2, ...b3 } as Country;
        });
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-[10px] text-muted-foreground text-center">
        Data: REST Countries API · Flags: FlagCDN · Map: OpenStreetMap — all free, no API key
      </p>
    </div>
  );
}
