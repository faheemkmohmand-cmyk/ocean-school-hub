...
const Admission = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ── 1. INSERT admission ──────────────────────────────────────────────────
      // No .select() here — anon has INSERT policy but NO SELECT policy on admissions.
      // Adding .select() after insert causes Supabase to hang indefinitely (RLS blocks it).
      const { error: insErr } = await supabase
        .from("admissions")
        .insert({
          reference_no:    "",               // DB trigger sets OHS-YYYY-XXXX
      ...
      // ── 2. GET reference_no via track_admission ──────────────────────────────
      // track_admission is SECURITY DEFINER + granted to anon — it bypasses RLS,
      // reads the row we just inserted, and returns reference_no safely.
      const { data: trackRows, error: trackErr } = await supabase
        .rpc("track_admission", { p_query: form.b_form_no.trim() });

      if (trackErr) throw new Error(`Could not fetch reference: ${trackErr.message}`);
      const latest = (trackRows ?? [])[0];
      if (!latest) throw new Error("Admission saved. Track it using your B-Form number.");

      const refNo = latest.reference_no;

      // ── 3. SAVE documents via SQL-patched RPC ───────────────────────────────
      // The RPC "save_admission_docs" (from admissions_patch.sql) is SECURITY DEFINER,
      // looks up the admission by b_form_no, and inserts the document rows.
      // If the SQL patch hasn't been run yet, this fails silently — docs are in
      // Cloudinary already and can be linked later by admin.
      ...
      if (docsPayload.length > 0) {
        await supabase.rpc("save_admission_docs", {
          p_b_form_no: form.b_form_no.trim(),
          p_docs:      JSON.stringify(docsPayload),
        });
        // Intentionally not throwing on error — admission is already saved.
        // Documents are in Cloudinary. Admin can see the URLs in Cloudinary dashboard.
      }
