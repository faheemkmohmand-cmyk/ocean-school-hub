...
const Admission = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // admissionIdRef now holds a temp_ string (never a real Supabase UUID).
      // We always go to the fresh-insert path — Supabase insert happens at Submit only.
      let admissionId: string | null = null;
      let refNo = "";

      if (false as boolean) {
        // Intentionally disabled — no pre-upload Supabase record is created anymore
        ...
        // Record any docs that were uploaded to temp folder (admissionId existed so they're already recorded)
      } else {
        // Direct insert — bypasses the missing submit_admission_public RPC entirely.
        // The "admissions_public_insert" RLS policy allows anon inserts directly.
        const { supabase } = await import("@/lib/supabase");

        // Use SECURITY DEFINER RPC — runs as DB owner, bypasses RLS on SELECT.
        // This single call inserts the admission, generates reference_no via trigger,
        // inserts all documents, and returns id + reference_no to anon safely.
        const docs = Object.entries(fileEntries)
        ...
        const { data: rpcData, error: admError } = await supabase
          .rpc("submit_admission_with_docs", {
            ...
            p_home_address:    form.home_address.trim() || null,
            p_gender:          form.gender || null,
            p_applying_class:  form.applying_class,
            p_admission_type:  form.admission_type,
            p_previous_school: form.previous_school.trim() || null,
            p_previous_class:  form.previous_class.trim() || null,
            p_previous_marks:  form.previous_marks.trim() || null,
            p_year_of_passing: form.year_of_passing.trim() || null,
            p_docs:            JSON.stringify(docs),
          });

        if (admError) throw new Error(`Submission failed: ${admError.message}`);
        admissionId = rpcData?.id ?? null;
        refNo = rpcData?.reference_no ?? "";

        // Documents are inserted inside the RPC above — nothing more to do here.
      }
