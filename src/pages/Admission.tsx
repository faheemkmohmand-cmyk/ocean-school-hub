...
const Admission = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // ── 1. INSERT admission (plain insert, no .select — anon has no SELECT policy) ──
      ...
      // ── 2. GET reference_no — track_admission is SECURITY DEFINER, granted to anon ──
      const { data: trackRows, error: trackErr } = await withTimeout(
        supabase.rpc("track_admission", { p_query: form.b_form_no.trim() }),
        15000,
        "Reference lookup"
      );
      if (trackErr) throw new Error(`Reference lookup failed: ${trackErr.message}`);
