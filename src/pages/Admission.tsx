...
const Admission = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Step 1: Save admission record to Supabase
      ...
      // Step 2: Upload documents ONE BY ONE to Cloudinary (sequential — avoids mobile hangs)
      const uploads = Object.entries(files).filter(([, f]) => f !== null);
      let done = 0;
      for (const [docType, file] of uploads) {
        setUploadStatus(`Uploading document ${done + 1} of ${uploads.length}…`);
        await uploadAdmissionDocument(result.id, docType, file as File);
        done++;
      }
                ...
                <div className="flex gap-3 mt-4">
                  ...
                  {step < 3 ? (
                    ...
                    <Button onClick={handleSubmit} disabled={submitting}
                      className="flex-1 gap-2 rounded-xl">
                      {submitting
                        ? <><Loader2 className="w-4 h-4 animate-spin" />{uploadStatus || "Submitting…"}</>
                        : "Submit Application"}
                    </Button>
                  )}
                </div>
