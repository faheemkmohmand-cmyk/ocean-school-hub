...
const Weather = () => {
  ...
  const LOC_KEY = "ghs_weather_loc_v1";
  const saveLoc = (lat: number, lon: number) => {
    try { localStorage.setItem(LOC_KEY, JSON.stringify({ lat, lon })); } catch { /**/ }
  };
  ...
  const detectLocation = useCallback(() => {
    ...
    watchId = navigator.geolocation.watchPosition(
      ({ coords: { latitude: lat, longitude: lon, accuracy: acc } }) => {
        ...
        // ≤ 150 m = real satellite fix — good enough, stop watching
        if (acc <= 150) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      try {
        const s = localStorage.getItem("ghs_weather_loc_v1");
              ...
              <div style={{display:"flex",gap:"7px",flexWrap:"wrap"}}>
                ...
                <button className="cbtn" title="Refresh weather"
                  onClick={() => {
                    ...
                    const saved = (() => {
                      try { const s = localStorage.getItem("ghs_weather_loc_v1"); return s ? JSON.parse(s) : null; } catch { return null; }
                    })();
                    const lat = saved?.lat ?? coords?.lat;
                    const lon = saved?.lon ?? coords?.lon;
