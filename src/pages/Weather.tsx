...
const Weather = () => {
  ...
  const fetchByIP = useCallback(async () => {
    try {
      // ip-api.com gives accurate city-level location for free, no key needed
    ...
    // Final fallback: Ghallanai, Pakistan (user's actual location)
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          ...
          // If accuracy > 200km (200000m), GPS is clearly wrong (e.g. VPN/ISP spoofing)
          // Fall back to IP-based location which is city-accurate
          if (acc > 200000) {
            await fetchByIP();
          } else {
        ...
        },
        {
          ...
          enableHighAccuracy: true // Request GPS, not cell-tower
        }
      );
    } else {
      fetchByIP();
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Score each result: prefer exact name match, prefer if query contains country code
      const queryLower = query.toLowerCase();
      const scored = data.map((r: { name: string; country: string; lat: number; lon: number; state?: string }) => {
        let score = 0;
        if (r.name.toLowerCase() === queryLower) score += 10;
        if (r.name.toLowerCase().includes(queryLower)) score += 5;
        // If user typed country code like "PK", "US" boost exact country
        if (queryLower.includes(r.country.toLowerCase())) score += 8;
        // Penalise results far from a Pakistan context if no country specified
        // (avoids defaulting to obscure European villages)
        if (!queryLower.includes(",") && r.country === "IE") score -= 6;
        return { ...r, score };
      });
          ...
          <div style={{ position: "relative", zIndex: 10, padding: "0 16px", maxWidth: "1000px", margin: "0 auto" }}>
            ...
            >
              ...
              <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                ...
                {/* My Location — re-triggers smart IP-based detection */}
                <button onClick={() => fetchByIP()} disabled={loading} title="Use my real location" style={{
                    background: "rgba(255,255,255,0.1)",
                    border: "1px solid rgba(255,255,255,0.2)",
                    borderRadius: "50px",
                    padding: "8px 14px",
                      ...
                      {showRadar && coords && (
                        <motion.div
                          ...
                          <iframe
                            ...
                            title="Weather Radar"
                            src={`https://embed.windy.com/embed2.html?lat=${coords.lat}&lon=${coords.lon}&detailLat=${coords.lat}&detailLon=${coords.lon}&width=1000&height=450&zoom=12&level=surface&overlay=rain&product=ecmwf&menu=&message=true&marker=true&calendar=now&pressure=&type=map&location=coordinates&detail=&metricWind=km%2Fh&metricTemp=%C2%B0C&radarRange=-1`}
                            style={{ width: "100%", height: "450px", border: "none", borderRadius: "20px" }}
                          />
                        </motion.div>
                      )}
                    </AnimatePresence>
