...
const WeatherBG = ({theme}:{theme:string}) => {
  const BG:Record<string,string> = {
    clear:"linear-gradient(135deg,#1a6dff,#00d4ff,#ff9a3c)",
    night:"linear-gradient(135deg,#0a0a2e,#1a1a4e,#2d1b69)",
    cloudy:"linear-gradient(135deg,#4a5568,#718096,#a0aec0)",
    rain:"linear-gradient(135deg,#1a1a2e,#2d3561,#1e4d8c)",
    snow:"linear-gradient(135deg,#e8f4fd,#c5dff8,#a3c4f0)",
    storm:"linear-gradient(135deg,#0a0a0a,#1a1a1a,#2d1b00)",
    fog:"linear-gradient(135deg,#c9d6df,#eeeeee,#c9d6df)",
  };
...
.glass{background:rgba(255,255,255,0.08);backdrop-filter:blur(22px);-webkit-backdrop-filter:blur(22px);border:1px solid rgba(255,255,255,0.14);border-radius:24px}
.lm .glass{background:rgba(255,255,255,0.72);border:1px solid rgba(0,0,0,0.08)}
.pill{background:rgba(255,255,255,0.09);border:1px solid rgba(255,255,255,0.13);border-radius:15px;padding:13px 8px;display:flex;flex-direction:column;align-items:center;gap:5px;text-align:center;transition:all .3s;cursor:default}
.pill:hover{background:rgba(255,255,255,0.17);transform:translateY(-3px)}
.lm .pill{background:rgba(255,255,255,0.72);border:1px solid rgba(0,0,0,0.07)}
.hcard{background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.12);border-radius:15px;padding:11px 9px;text-align:center;min-width:70px;flex-shrink:0;transition:all .3s}
.hcard:hover{background:rgba(255,255,255,0.15);transform:translateY(-4px)}
.lm .hcard{background:rgba(255,255,255,0.72);border:1px solid rgba(0,0,0,0.07)}
.scr::-webkit-scrollbar{height:4px}.scr::-webkit-scrollbar-track{background:rgba(255,255,255,0.04)}.scr::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.25);border-radius:4px}
...
.drow:hover{background:rgba(255,255,255,0.08)}
.lm .drow:hover{background:rgba(0,0,0,0.04)}
.big-icon{line-height:1;filter:drop-shadow(0 4px 24px rgba(0,0,0,0.3));animation:floatIcon 4s ease-in-out infinite}
.srch{background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:50px;padding:10px 42px 10px 42px;color:white;outline:none;width:100%;font-size:14px;backdrop-filter:blur(10px);transition:all .3s}
.srch::placeholder{color:rgba(255,255,255,0.45)}.srch:focus{border-color:rgba(255,255,255,0.5);background:rgba(255,255,255,0.15)}
.lm .srch{background:rgba(255,255,255,0.8);border:1px solid rgba(0,0,0,0.14);color:#1a1a2e}.lm .srch::placeholder{color:rgba(0,0,0,0.38)}
/* Suggestions */
.sug-box{position:absolute;top:calc(100% + 6px);left:0;right:0;background:rgba(12,12,30,0.98);backdrop-filter:blur(28px);border:1px solid rgba(255,255,255,0.13);border-radius:18px;overflow:hidden;z-index:9999;max-height:300px;overflow-y:auto;box-shadow:0 24px 64px rgba(0,0,0,0.6)}
.lm .sug-box{background:rgba(255,255,255,0.99);border:1px solid rgba(0,0,0,0.1);box-shadow:0 12px 40px rgba(0,0,0,0.15)}
.sug-item{padding:11px 15px;cursor:pointer;display:flex;align-items:flex-start;gap:10px;border-bottom:1px solid rgba(255,255,255,0.055);transition:background .18s}
.sug-item:last-child{border-bottom:none}
.sug-item:hover,.sug-item:active{background:rgba(96,165,250,0.22)}
...
/* Ctrl buttons */
.cbtn{background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:50px;padding:8px 13px;color:white;cursor:pointer;backdrop-filter:blur(10px);display:flex;align-items:center;gap:5px;font-size:12px;font-weight:600;transition:all .3s;white-space:nowrap}
...
const Weather = () => {
  ...
  // This is the fix for "shows Tangi instead of Ghallanai".
  // OWM's city database is coarse; Nominatim has every village.
  const reverseGeocode = useCallback(async (lat:number, lon:number) => {
    try {
      const r = await fetch(
        `https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lon}&format=json&zoom=14&accept-language=en`,
        { headers:{"User-Agent":"OceanSchoolHub/1.0 (school weather project)"} }
      );
      if (!r.ok) return;
      const d = await r.json();
      // Prefer smallest administrative unit (village > hamlet > town > city > county)
      const place =
        d.address?.village  ||
        d.address?.hamlet   ||
        d.address?.suburb   ||
