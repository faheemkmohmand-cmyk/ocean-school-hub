...
/** API returns Celsius (units=metric), convert to F if needed */
const toCelsius = (c: number) => Math.round(c);
...
const Weather = () => {
  ...
  const [radarLayer, setRadarLayer] = useState<"rain"|"satellite"|"wind"|"clouds"|"temp">("rain");
  ...
  const fetchWeatherData = useCallback(async (lat: number, lon: number) => {
    ...
    try {
      // Current weather
      // Cache-busting: changes every minute so browser never serves stale data
      const cacheBust = Math.floor(Date.now() / 60000);
      const [curRes, foreRes, aqiRes] = await Promise.all([
        fetch(`${BASE_URL}/weather?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=metric&_=${cacheBust}`),
        fetch(`${BASE_URL}/forecast?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=metric&cnt=56&_=${cacheBust}`),
        fetch(`${BASE_URL}/air_pollution?lat=${lat}&lon=${lon}&appid=${API_KEY}&_=${cacheBust}`),
      ]);
      ...
      try {
        const uvRes = await fetch(`${BASE_URL}/uvi?lat=${lat}&lon=${lon}&appid=${API_KEY}&_=${cacheBust}`);
        if (uvRes.ok) {
  ...
  // ── IP-based geolocation (most reliable fallback) ─────────────────────────
  const fetchByIP = useCallback(async () => {
    // Try HTTPS ipapi.co first (avoids mixed-content errors on https sites)
    try {
    ...
    // Fallback: ip-api via http (works on non-https pages)
    try {
      const res2 = await fetch("https://ip-api.com/json/?fields=lat,lon,city,country,status");
      const data2 = await res2.json();
      if (data2.status === "success" && data2.lat) {
        await fetchWeatherData(data2.lat, data2.lon);
        return true;
      }
    } catch { /* silent */ }
    // Final fallback: Ghallanai, Pakistan
    await fetchWeatherData(34.4907, 71.5275);
    return false;
  }, [fetchWeatherData]);
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          ...
          const acc = pos.coords.accuracy; // accuracy in metres

          // Accept GPS if accuracy < 50km (50000m)
          // This rejects VPN/spoofed coordinates but accepts real GPS
          if (acc < 50000) {
            await fetchWeatherData(lat, lon);
          } else {
            // GPS too inaccurate, use IP geolocation (city-accurate)
            await fetchByIP();
          }
        },
        async () => {
          // GPS denied or failed → use IP geolocation
          await fetchByIP();
        },
        {
          timeout: 15000,          // 15s timeout for GPS to acquire signal
          maximumAge: 0,           // NEVER use cached position — always fresh
          enableHighAccuracy: true // Use GPS chip, not just cell towers
        }
      );
    } else {
      fetchByIP();
    }
  }, [fetchWeatherData, fetchByIP]);

  // Search by city name — returns top 5 results, picks best match
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      // Get up to 5 results so we can find the best match
      ...
      if (!data || data.length === 0) {
        setError(`"${query}" not found. Try adding a country, e.g. "Ghallanai, PK"`);
        return;
      }

      const queryLower = query.toLowerCase();
      // Parse optional country code from "City, Country" e.g. "London, GB"
      const queryParts = queryLower.split(",").map((s: string) => s.trim());
      const queryCity    = queryParts[0];
      const queryCountry = queryParts[1] || ""; // e.g. "pk", "us"

      const scored = data.map((r: { name: string; country: string; lat: number; lon: number; state?: string }) => {
        let score = 0;
        const nameLower    = r.name.toLowerCase();
        const countryLower = r.country.toLowerCase();

        // City name matching
        if (nameLower === queryCity) score += 20;           // exact match = highest
        else if (nameLower.startsWith(queryCity)) score += 10;
        else if (nameLower.includes(queryCity)) score += 5;

        if (queryCountry) {
          // User specified country — heavily weight it
          if (countryLower === queryCountry) score += 25;
          else if (queryCountry.includes(countryLower)) score += 10;
          else score -= 20; // wrong country when user specified = strong penalty
        } else {
          // No country specified — apply regional heuristics
          if (countryLower === "pk") score += 8;   // Pakistan preference for this app
          if (countryLower === "ie") score -= 15;  // Ireland penalty (was causing bug)
          // Prefer major countries
          const common = ["pk","in","us","gb","ae","sa","bd","ng","eg","tr"];
          if (common.includes(countryLower)) score += 3;
        }
        return { ...r, score };
      });
          ...
          <div style={{ position: "relative", zIndex: 10, padding: "0 16px", maxWidth: "1000px", margin: "0 auto" }}>
            ...
            >
              ...
              <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                ...
                {/* My Location — triggers real GPS first, then IP fallback */}
                ...
                }} disabled={loading} title="Use my exact GPS location" style={{
                    background: "rgba(255,255,255,0.1)",
                    border: "1px solid rgba(255,255,255,0.2)",
                    borderRadius: "50px",
                    padding: "8px 14px",
            ...
            {!loading && current && (
              <AnimatePresence mode="wait">
                <motion.div key={current.name}
                  ...
                  {/* ── Radar Map — FIXED: satellite + proper zoom ── */}
                  <motion.div className="glass-card"
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px", flexWrap: "wrap", gap: "10px" }}>
                      ...
                      <div style={{ display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap" }}>
                        {showRadar && (
                          <>
                            {(["rain","satellite","wind","clouds","temp"] as const).map((layer) => {
                              const labels: Record<string, string> = { rain:"🌧️ Radar", satellite:"🛰️ Satellite", wind:"💨 Wind", clouds:"☁️ Clouds", temp:"🌡️ Temp" };
                              return (
                                <button key={layer}
                                  onClick={() => setRadarLayer(layer)}
                                  style={{
                                    background: radarLayer === layer ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.1)",
                                    border: radarLayer === layer ? "1px solid rgba(255,255,255,0.6)" : "1px solid rgba(255,255,255,0.2)",
                        ...
                        <button onClick={() => setShowRadar(r => !r)} style={{
                          background: showRadar ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.1)",
                    ...
                    <AnimatePresence>
                      {showRadar && coords && (
                        <motion.div
                          ...
                          <iframe
                            title={`Weather Radar - ${radarLayer}`}
                            src={`https://embed.windy.com/embed2.html?lat=${coords.lat.toFixed(4)}&lon=${coords.lon.toFixed(4)}&detailLat=${coords.lat.toFixed(4)}&detailLon=${coords.lon.toFixed(4)}&width=1000&height=450&zoom=7&level=surface&overlay=${radarLayer}&product=ecmwf&menu=&message=true&marker=true&calendar=now&pressure=&type=map&location=coordinates&detail=&metricWind=km%2Fh&metricTemp=%C2%B0C&radarRange=-1`}
                            style={{ width: "100%", height: "450px", border: "none", borderRadius: "20px", display: "block", background: "#1a1a2e" }}
                            allow="geolocation"
                            loading="lazy"
                          />
                        </motion.div>
                    ...
                    {!showRadar && (
                      <div style={{
                        ...
                        <div style={{ textAlign: "center" }}>
                          ...
                          <p className={subColor} style={{ fontSize: "14px", marginTop: "8px" }}>Click "Show Radar" · Switch between Radar / Satellite / Wind / Clouds</p>
                        </div>
                      </div>
                    )}
                  </motion.div>
