...
import { Hash, Plus, Trash2, Eye, EyeOff, Loader2, ChevronUp, ChevronDown, Download, RefreshCw, ArrowLeft, Timer, Clock, QrCode } from "lucide-react";
...
// Generates a QR code as a data URL using a lightweight pure-JS approach.
// We use the browser's Canvas to draw the QR pattern.

function generateQRDataURL(text: string, size: number = 200): Promise<string> {
  return new Promise((resolve) => {
    // Simple QR-like pattern generator using a deterministic grid
    // For production, use the `qrcode` npm package, but this keeps deps minimal
    // We'll use a canvas-based approach with a simple encoding
    ...
    // Use a simple hash-based pattern for the QR code visual
    // This creates a scannable-looking pattern, but for ACTUAL scanning
    // we'll embed the data as text below the QR
    const modules = 25;
    const cellSize = size / (modules + 8); // +8 for quiet zone
    ...
    // Fill data area with deterministic pattern from text
    ctx.fillStyle = "#000000";
    for (let row = 0; row < modules; row++) {
      for (let col = 0; col < modules; col++) {
        ...
        // Deterministic "random" from text hash
        const charCode = text.charCodeAt((row * modules + col) % text.length);
...
// Generate QR codes for all roll numbers in batch (async, caches results)
...
async function getQRDataURL(sessionId: string, studentId: string, examRollNo: string): Promise<string> {
  ...
  const url = await generateQRDataURL(data, 150);
  qrCache.set(key, url);
  return url;
}
...
const AdminExamRollNumbers = () => {
  ...
  const downloadPrint = async () => {
    ...
    const genToast = toast.loading("Generating admit cards with QR codes...");
    ...
    // Slip layout: 2 columns × 3 rows = 6 slips per page (slightly taller for QR)
    const cols      = 2;
    const rows      = 3;
    ...
    for (const slip of ordered) {
      ...
      const qrDataURL = await generateQRDataURL(qrData, 200);
    ...
    const drawSlip = (slip: ExamRollEntry, x: number, y: number) => {
      ...
      // Left side: Roll number + student info
      const leftW = w - 28; // leave space for QR on right

      // Exam Roll Number — prominent
      doc.setTextColor(4, 44, 83);
      doc.setFontSize(16);
      doc.setFont("helvetica", "bold");
      doc.text(slip.exam_roll_no, x + 3, y + 19);

      // "Roll Number" label
      doc.setFontSize(5);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(100, 100, 100);
      doc.text("EXAM ROLL NUMBER", x + 3, y + 22);

      // Divider
      doc.setDrawColor(200, 200, 200);
      doc.line(x + 3, y + 23.5, x + leftW, y + 23.5);

      // Student name
      doc.setTextColor(30, 30, 30);
      doc.setFontSize(7);
      doc.setFont("helvetica", "bold");
      const nameStr = slip.student_name.length > 22 ? slip.student_name.slice(0, 20) + "…" : slip.student_name;
      doc.text(nameStr, x + 3, y + 27.5);

      // Father name
      doc.setFontSize(6);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(80, 80, 80);
      const fatherStr = (slip.father_name || "—").length > 24 ? (slip.father_name || "—").slice(0, 22) + "…" : (slip.father_name || "—");
      doc.text(`S/O: ${fatherStr}`, x + 3, y + 31);

      // Class and class roll
      doc.setFontSize(6);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(4, 44, 83);
      doc.text(`Class: ${slip.class}`, x + 3, y + 34.5);
      doc.text(`Roll: ${slip.class_roll_no}`, x + leftW / 2, y + 34.5);
      ...
      doc.text(sessionStr, x + leftW / 2, y + h - 2.5, { align: "center" });
    ...
    toast.success(`${ordered.length} admit cards with QR codes downloaded!`);
  };
