...
// All-classes PDF — one page per class + summary page
function generateAllClassesPDF(byClass: Record<string, MeritEntry[]>, year: number) {
  ...
  // Summary page first
  drawPDFHeader(doc, "WHOLE SCHOOL MERIT LIST", `Year ${year}  |  All Classes  |  Summary`, w);
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(10, 10, 10);
  doc.text("Class-wise Summary", w / 2, 52, { align: "center" });

  const summaryBody = sortedClasses.map(cls => {
    ...
    const examType = entries[0]?.exam_type || "-";
    const passRate = entries.length ? Math.round((passing.length / entries.length) * 100) : 0;
    return [
      `Class ${cls}`,
      examType,
  ...
  autoTable(doc, {
    startY: 56,
    head: [["Class", "Exam Type", "Total", "Passed", "Pass Rate", "Highest %", "Avg %", "1st Position"]],
    body: summaryBody,
    headStyles: { fillColor: [4, 44, 83], textColor: [255, 255, 255], fontSize: 8, fontStyle: "bold", halign: "center", cellPadding: 3 },
    bodyStyles: { fontSize: 8.5, cellPadding: 3, textColor: [20, 20, 20] },
    alternateRowStyles: { fillColor: [246, 247, 250] },
    columnStyles: {
  ...
  for (const cls of sortedClasses) {
    ...
    doc.addPage();
    const examType = entries[0]?.exam_type || "N/A";
    drawPDFHeader(doc, `MERIT LIST — CLASS ${cls}`, `${examType}  |  Year ${year}  |  ${entries.length} Students`, w);
    ...
    autoTable(doc, {
      ...
      headStyles: { fillColor: [4, 44, 83], textColor: [255, 255, 255], fontStyle: "bold", fontSize: 8.5, halign: "center", cellPadding: 3.5 },
      bodyStyles: { fontSize: 8.5, cellPadding: 3, textColor: [20, 20, 20] },
      alternateRowStyles: { fillColor: [246, 247, 250] },
      columnStyles: {
        0: { halign: "center", cellWidth: 14, fontStyle: "bold" },
...
function SchoolMeritTab() {
  const [yearInput, setYearInput] = useState(String(new Date().getFullYear()));
  const [viewMode, setViewMode] = useState<"class"|"combined">("class");
  ...
  // All entries combined and ranked
  ...
  // Entries to display based on filter
  const displayEntries = useMemo(() => {
    if (viewMode === "combined") return allEntries;
    if (selectedClass === "all") return allEntries;
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex-1 min-w-[130px]">
            ...
            <Select value={viewMode} onValueChange={v => setViewMode(v as any)}>
              ...
              <SelectContent>
                ...
                <SelectItem value="combined">Combined Ranking</SelectItem>
              </SelectContent>
            </Select>
          </div>
          {viewMode === "class" && (
            <div className="flex-1 min-w-[100px]">
              ...
              <Select value={selectedClass} onValueChange={setSelectedClass}>
                ...
                <SelectContent>
                  ...
                  {sortedClasses.map(c => <SelectItem key={c} value={c}>Class {c} ({byClass[c]?.length || 0})</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          )}
      ...
      {!isLoading && totalStudents > 0 && (
        <div className="flex gap-2 flex-wrap items-center">
          <Button onClick={() => generateAllClassesPDF(byClass, year)} variant="outline" className="gap-1.5">
            <Download className="w-4 h-4" /> Download Full School PDF
          </Button>
          <span className="text-xs text-muted-foreground self-center">
            {sortedClasses.length} class{sortedClasses.length !== 1 ? "es" : ""} · {totalStudents} students · {passing.length} passed
          </span>
        </div>
      )}
      ...
      {!isLoading && sortedClasses.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {sortedClasses.map(cls => {
            const e = byClass[cls] || [];
            const p = e.filter(x => x.is_pass || x.percentage >= 33);
            const avg = e.length ? Math.round(e.reduce((s, x) => s + x.percentage, 0) / e.length) : 0;
            return (
              <div key={cls}
                onClick={() => { setSelectedClass(cls); setViewMode("class"); }}
                className="bg-card border border-border rounded-xl p-3 text-center cursor-pointer hover:border-primary/50 hover:shadow-md transition-all">
                <p className="text-sm font-bold text-foreground">Class {cls}</p>
                <p className="text-xs text-muted-foreground">{e.length} students</p>
                <p className="text-xs text-green-600 font-semibold">{e.length ? Math.round((p.length/e.length)*100) : 0}% pass</p>
      ...
      {!validYear ? (
        ...
        <MeritTable entries={displayEntries} showClass={viewMode === "combined" || selectedClass === "all"} />
      )}
    </div>
  );
}
