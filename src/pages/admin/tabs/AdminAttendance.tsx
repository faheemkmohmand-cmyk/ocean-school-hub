...
import {
  ...
  Users, TrendingUp, AlertTriangle, FileText, Printer, ArrowUpDown,
} from "lucide-react";
import {
...
const statusConfig: Record<Status, {
  ...
  ring: string;           // ring when active
  soft: string;           // soft bg for inactive button
  text: string;           // colored text token
}> = {
  present: {
...
const FILTERS = ["all", "present", "absent", "late", "leave"] as const;
...
const AdminAttendance = () => {
  ...
  const alreadySaved = existingAttendance.length > 0;
  ...
  const reportSummary = useMemo(() => {
    if (!reportData.length) return { avg: 0, low: 0, perfect: 0 };
  ...
  const exportExcel = () => {
    ...
    const wsData = [
      ...
      [`Average Attendance: ${reportSummary.avg}%   |   Below 75%: ${reportSummary.low}   |   Perfect (100%): ${reportSummary.perfect}`],
      [],
      ["Roll No", "Student Name", "Present", "Absent", "Late", "Leave", "Working Days", "Attendance %"],
    ...
    ws["!cols"] = [{ wch: 10 }, { wch: 26 }, { wch: 10 }, { wch: 10 }, { wch: 8 }, { wch: 8 }, { wch: 14 }, { wch: 14 }];
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(10);
    doc.text(
      `Average: ${reportSummary.avg}%    Below 75%: ${reportSummary.low}    Perfect: ${reportSummary.perfect}    Working days: ${monthDays.length}`,
      14, 23,
    );
    autoTable(doc, {
      startY: 28,
      head: [["Roll", "Name", "P", "A", "L", "V", "Days", "%"]],
      body: reportData.map((r) => [r.roll_number, r.full_name, r.present, r.absent, r.late, r.leave, r.total, `${r.pct}%`]),
      styles: { fontSize: 9 },
  ...
  const handleTabChange = useCallback((v: string) => setTab(v as "mark" | "report"), []);
  ...
  return (
    <div className="space-y-4 pb-28 md:pb-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Tabs value={tab} onValueChange={handleTabChange} className="flex-1 min-w-[14rem]">
          <TabsList className="w-full sm:w-auto">
            <TabsTrigger value="mark" className="flex-1 sm:flex-none">Mark</TabsTrigger>
        ...
        <div className="space-y-4">
          ...
          {total > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
              {(["present", "absent", "late", "leave"] as Status[]).map((s) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-lg border border-border bg-card p-3 text-left transition-all ${
                      filter === s ? "ring-2 ring-primary" : "hover:border-primary/40"
                    }`}
                  >
                    <div className={`text-[11px] font-medium uppercase tracking-wide ${cfg.text}`}>
                      {cfg.label}
                    </div>
              ...
              <div className="rounded-lg border border-primary/30 bg-primary/5 p-3 col-span-2 sm:col-span-1">
                <div className="text-[11px] font-medium uppercase tracking-wide text-primary">
                  Attendance
                </div>
                <div className="mt-1 flex items-baseline gap-1.5">
                  <span className="text-xl font-bold">{presentPct}%</span>
                </div>
                <div className="mt-1.5 h-1 rounded-full bg-muted overflow-hidden">
          ...
          {filter !== "all" && (
            <div className="text-xs text-muted-foreground">
              Filtering by <span className="font-medium text-foreground capitalize">{filter}</span>.{" "}
                        ...
                        {STATUS_ORDER.map((opt) => {
                          ...
                          return (
                            <button
                              ...
                              aria-label={o.label}
                              aria-pressed={active}
                              className={`h-9 rounded-md flex items-center justify-center gap-1 text-xs font-semibold transition-all ${
                                active ? o.ring : o.soft
                              }`}
                            >
                              {o.icon}
                              <span className="hidden xs:inline">{o.short}</span>
                            </button>
                          );
                        })}
                      </div>
          ...
          {/* Sticky save bar */}
          <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-background/95 backdrop-blur p-3 md:static md:bg-transparent md:border-0 md:p-0 md:pt-2">
            <div className="flex items-center justify-between gap-2 max-w-screen-xl mx-auto">
              <div className="text-xs text-muted-foreground hidden sm:block">
                {isDirty ? (
                  ...
                  <span>Tap a student's P / A / L / V</span>
                )}
              </div>
              <Button
                onClick={handleSave}
