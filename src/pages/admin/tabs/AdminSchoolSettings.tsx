...
// ✅ ImageUploader — uploads to Cloudinary with automatic Supabase Storage fallback
const ImageUploader = ({
  ...
  // Clear error when user starts a new upload
  const clearError = () => {
    setUploadError(null);
    setRetryCount(0);
  };
  ...
  const doUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      setUploadError(msg);
      setPreview(currentUrl);
      // Show a shorter toast, full error is shown in the component
      toast.error(`${label} upload failed. See details below.`, { duration: 5000 });
    }
...
const AdminSchoolSettings = () => {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {/* Important: Remind admin to save after uploading images */}
      <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 flex items-start gap-2">
        <AlertTriangle className="w-4 h-4 text-primary shrink-0 mt-0.5" />
        <p className="text-sm text-muted-foreground">
          After uploading your logo or banner, remember to click <strong>"Save All Changes"</strong> below
          to store the URLs in the database. Otherwise the images won't appear on the public website.
        </p>
      </div>
