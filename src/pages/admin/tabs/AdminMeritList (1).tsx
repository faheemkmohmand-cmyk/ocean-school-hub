...
function drawPDFHeader(doc: jsPDF, title: string, subtitle: string, w: number) {
  ...
  doc.text("District Mohmand, KPK  |  Established 2018", w / 2, 20, { align: "center" });
  // Gold divider
...
function generateClassPDF(entries: MeritEntry[], cls: string, examType: string, year: number) {
  ...
  doc.setFontSize(8);
  doc.setFont("helvetica", "bold");
  doc.text(`Passed: ${passing.length}/${entries.length}`, 20, 58);
  doc.text(`Highest: ${highest}%`, 65, 58);
  doc.text(`Class Avg: ${avg}%`, 105, 58);
  doc.text(`Pass Rate: ${entries.length ? Math.round((passing.length / entries.length) * 100) : 0}%`, 148, 58);
