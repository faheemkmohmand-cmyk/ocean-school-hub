...
function useSchoolToppers() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const toppers: any[] = [];
      for (const r of (data ?? [])) {
        if (!seen.has(r.class)) { seen.add(r.class); toppers.push(r); }
      }
      return toppers.sort((a, b) => Number(a.class) - Number(b.class));
    },
...
const TOPPER_GRADIENTS = [
  "from-[#1a6fb5] via-[#1e90d4] to-[#5bc4f5]",   // Class 6 - blue
  "from-[#6c3fc5] via-[#8b5cf6] to-[#c084fc]",   // Class 7 - purple
  "from-[#0e7a5a] via-[#10b981] to-[#6ee7b7]",   // Class 8 - green
  "from-[#c2410c] via-[#ea580c] to-[#fb923c]",   // Class 9 - orange
  "from-[#1d4ed8] via-[#3b82f6] to-[#93c5fd]",   // Class 10 - indigo
];
    ...
    <section className="py-16 bg-gradient-to-b from-background to-primary/5">
      <div className="container mx-auto px-4">
        <div className="text-center mb-10">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Rank #1 students from latest published results</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 max-w-5xl mx-auto">
            {toppers.map((t, i) => {
              ...
              const gradient = TOPPER_GRADIENTS[i % TOPPER_GRADIENTS.length];
              return (
                <motion.div
                  key={i}
                ...
                >
                  <div className={`relative rounded-3xl overflow-hidden shadow-xl bg-gradient-to-b ${gradient} text-white`}>
                    {/* Top gradient photo area */}
                    <div className="relative h-40 flex items-center justify-center pt-6 pb-4">
                      {/* Crown for #1 */}
                      ...
                      {photoUrl ? (
                        <img
                          src={photoUrl}
                          alt={name}
                          className="w-20 h-20 rounded-full object-cover border-4 border-white/40 shadow-lg mt-4"
                        />
                      ) : (
                        <div className="w-20 h-20 rounded-full bg-white/20 backdrop-blur-sm border-4 border-white/40 flex items-center justify-center text-3xl font-black shadow-lg mt-4">
                          {initials}
                        </div>
                      )}

                      {/* Rank badge */}
                      <div className="absolute top-3 right-3 bg-white/20 backdrop-blur-sm rounded-full px-2 py-0.5 text-[10px] font-black border border-white/30">
                        #1
                      </div>
                    </div>

                    {/* Info panel — white card at bottom */}
                    <div className="bg-white/10 backdrop-blur-sm mx-2 mb-2 rounded-2xl p-3 text-center">
                      <h3 className="text-sm font-black text-white leading-tight truncate">{name}</h3>
                      <p className="text-[10px] text-white/70 mt-0.5 font-medium">Class {t.class}</p>

                      {/* Stats row */}
                      <div className="flex items-center justify-center gap-2 mt-2">
                        <div className="flex items-center gap-1 bg-white/20 rounded-lg px-2 py-1">
                          <span className="text-sm font-black">{t.percentage?.toFixed(0)}%</span>
                        </div>
                        <div className="flex items-center gap-1 bg-white/20 rounded-lg px-2 py-1">
                          <span className="text-sm font-black">{t.grade || "A+"}</span>
                        </div>
                      </div>
                      <p className="text-[9px] text-white/60 mt-1.5">{t.exam_type} · {t.year}</p>
                    </div>
                  </div>
                </motion.div>
              );
  ...
  return (
    <PageLayout>
      ...
      <section id="hero-section" className="relative min-h-[100vh] flex items-center">
        ...
        {/* ── Scroll Indicator — three bouncing chevrons ── */}
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-1 select-none pointer-events-none">
          <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/50 mb-1">Scroll</span>
          {[0, 1, 2].map((i) => (
            <motion.div
              key={i}
            ...
            >
              <svg width="16" height="10" viewBox="0 0 16 10" fill="none">
                <path d="M1 1L8 8L15 1" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" strokeOpacity="0.8"/>
              </svg>
            </motion.div>
          ))}
        </div>
