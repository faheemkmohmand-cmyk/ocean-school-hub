...
/* ─── ScrollReveal wrapper — from reference, slides in from direction ─── */
...
/* ─── 3D Tilt Card — mouse-tracking perspective tilt ─── */
...
/* ─── Floating Orb — decorative animated circle ─── */
function FloatingOrb({ size, color, top, left, right, bottom, delay = 0 }: {
  size: string; color: string; top?: string; left?: string; right?: string; bottom?: string; delay?: number;
}) {
  return (
    <motion.div
      className="absolute rounded-full pointer-events-none"
      style={{ width: size, height: size, background: color, top, left, right, bottom, filter: "blur(60px)" }}
...
/* ─── Animated 3D button wrapper ─── */
function AnimButton({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <motion.div
    ...
    <PageLayout>
      ...
      <section id="hero-section" className="relative min-h-[100vh] flex items-center overflow-hidden">
        ...
        >
          ...
          />
          {/* Extra orbs from reference */}
          <motion.div
            animate={{ scale: [1, 1.3, 1], x: [0, 50, 0], y: [0, -20, 0], rotate: [0, 180, 360] }}
            transition={{ duration: 22, repeat: Infinity, ease: "linear" }}
            className="absolute top-1/4 right-1/4 w-72 h-72 rounded-full bg-sky-400/10 blur-3xl"
          />
          <motion.div
            animate={{ scale: [1.2, 1, 1.2], x: [0, -30, 0], y: [0, 35, 0] }}
            transition={{ duration: 18, repeat: Infinity, ease: "easeInOut" }}
            className="absolute top-1/2 left-1/3 w-56 h-56 rounded-full bg-violet-400/10 blur-3xl"
          />
          {/* Animated floating dots */}
          {[
            { top: "20%", left: "15%", delay: 0 },
            { top: "35%", right: "20%", delay: 1.2 },
            { bottom: "25%", left: "40%", delay: 0.6 },
            { top: "60%", right: "10%", delay: 1.8 },
            { top: "10%", right: "35%", delay: 0.9 },
          ].map((pos, i) => (
            <motion.div
              key={i}
              className="absolute w-2 h-2 rounded-full bg-white/30"
              style={{ top: pos.top, left: (pos as any).left, right: (pos as any).right, bottom: (pos as any).bottom }}
        ...
        <div className="absolute right-8 top-1/2 -translate-y-1/2 hidden xl:flex flex-col gap-4 z-10">
          {[
            { emoji: "🎓", label: "Pass Rate", value: "98%", delay: 0 },
            { emoji: "📚", label: "Study Notes", value: "Live", delay: 0.2 },
            { emoji: "🏆", label: "Top Results", value: "A+", delay: 0.4 },
          ].map((card, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, x: 60, rotateY: -20 }}
              animate={{ opacity: 1, x: 0, rotateY: 0 }}
              transition={{ delay: 0.8 + card.delay, duration: 0.7, type: "spring", stiffness: 120 }}
              whileHover={{ scale: 1.05, x: -4, rotateY: 5 }}
              style={{ transformStyle: "preserve-3d", transformPerspective: 600 }}
              className="bg-white/15 backdrop-blur-xl border border-white/20 rounded-2xl px-4 py-3 flex items-center gap-3 shadow-2xl cursor-default"
            >
              <span className="text-2xl">{card.emoji}</span>
              <div>
                <p className="text-white font-black text-lg leading-none">{card.value}</p>
                <p className="text-white/60 text-[11px] font-medium">{card.label}</p>
              </div>
            </motion.div>
          ))}
        </div>
          ...
          >
            ...
            <motion.div variants={stagger.child} className="mt-8 flex flex-wrap gap-4">
              <AnimButton>
                <Link
                  to="/results"
                  className="inline-flex items-center gap-2 gradient-accent text-primary-foreground font-semibold px-7 py-3.5 rounded-xl shadow-elevated hover:shadow-card transition-all duration-200"
                >
                  View Results
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.15 }} variants={sectionFadeUp} className="py-20 cv-auto">
        <div className="container mx-auto px-4">
          ...
          >
            {features.map((f, idx) => (
              <ScrollReveal key={f.title} delay={idx * 0.08} direction="up">
                <TiltCard>
                  <motion.div
                    variants={stagger.child}
                    whileHover={{ y: -8 }}
                  ...
                  >
                    ...
                    <h3 className="font-heading font-semibold text-foreground text-lg" style={{ transform: "translateZ(12px)" }}>{f.title}</h3>
              ...
              : news.map((item, idx) => (
                  <ScrollReveal key={item.id} delay={idx * 0.1} direction="up">
                    <TiltCard className="h-full">
                      <motion.div
                        whileHover={{ y: -6 }}
                        className="bg-card rounded-2xl overflow-hidden shadow-card hover:shadow-elevated transition-all duration-300 group h-full"
                        style={{ transformStyle: "preserve-3d" }}
                      >
                        ...
                        <div className="p-5" style={{ transform: "translateZ(8px)" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {teachersLoading
              ...
              : teachers.map((teacher, idx) => (
                  <ScrollReveal key={teacher.id} delay={idx * 0.1} direction="up">
                    <TiltCard>
                      <motion.div
                        whileHover={{ y: -8 }}
                        className="bg-card rounded-2xl p-6 text-center shadow-card hover:shadow-elevated transition-all duration-300 group h-full"
                        style={{ transformStyle: "preserve-3d" }}
                      >
                        {teacher.photo_url ? (
                          <img
                          ...
                          <div className="w-20 h-20 rounded-full mx-auto mb-4 gradient-hero flex items-center justify-center text-primary-foreground text-xl font-heading font-bold" style={{ transform: "translateZ(16px)" }}>
                            {teacher.full_name.split(" ").map(n => n[0]).join("").slice(0, 2)}
                          </div>
                        )}
                        <h3 className="font-heading font-semibold text-foreground" style={{ transform: "translateZ(10px)" }}>{teacher.full_name}</h3>
                        {teacher.subject && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {achievementsLoading
              ...
              : achievements.map((a, idx) => (
                  <ScrollReveal key={a.id} delay={idx * 0.12} direction="up">
                    <TiltCard className="h-full">
                      <motion.div
                        whileHover={{ y: -6 }}
                        className="bg-card rounded-2xl p-6 shadow-card hover:shadow-elevated transition-all duration-300 h-full"
                        style={{ transformStyle: "preserve-3d" }}
                      >
                        <motion.div
                          className="w-12 h-12 rounded-xl bg-warning/15 flex items-center justify-center mb-4"
                          style={{ transform: "translateZ(16px)" }}
                        ...
                        <h3 className="font-heading font-semibold text-foreground" style={{ transform: "translateZ(10px)" }}>{a.title}</h3>
                        {a.student_name && (
          ...
          >
            {[
              ...
              <motion.div key={item.label} variants={stagger.child}>
                <ScrollReveal delay={idx * 0.07} direction="up">
                  <TiltCard>
                    <AnimButton className="w-full">
                      <Link
                        to={item.to}
                        className="w-full bg-card rounded-2xl p-5 shadow-card hover:shadow-elevated transition-all duration-300 text-center group block"
                        style={{ transformStyle: "preserve-3d" }}
                      >
                        <div
                          className="w-12 h-12 rounded-xl gradient-hero flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform"
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.15 }} variants={sectionFadeUp} className="py-20">
        <div className="container mx-auto px-4">
          <div className="gradient-hero rounded-3xl p-12 md:p-20 text-center relative overflow-hidden">
            ...
            {/* Extra animated orbs */}
            <FloatingOrb size="200px" color="rgba(255,255,255,0.07)" top="-40px" left="-40px" delay={0} />
            <FloatingOrb size="160px" color="rgba(255,255,255,0.06)" bottom="-30px" right="-30px" delay={1.5} />
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
            ...
            >
              ...
              <AnimButton className="mt-8">
                <Link
                  to="/results"
                  className="inline-flex items-center gap-2 bg-white text-primary-dark font-bold px-8 py-4 rounded-xl shadow-elevated text-lg"
                >
                  Check Results
