  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("results")
        .select("class, exam_type, year, obtained_marks, total_marks, percentage, grade, students(full_name, roll_number, photo_url)")
              ...
              return (
                ...
                >
                  <div className={`relative rounded-3xl p-6 text-center shadow-elevated overflow-hidden border-2 ${
                    ...
                    {/* Avatar with photo */}
                    {(t.students as any)?.photo_url ? (
                      <img
                        src={(t.students as any).photo_url}
                        alt={name}
                        className={`w-20 h-20 rounded-full mx-auto mb-3 object-cover shadow-lg border-4 border-white ${
                          isFirst ? "ring-4 ring-amber-300 ring-offset-2 mt-0 w-24 h-24" : "mt-6"
                        }`}
                      />
                    ) : (
                      <div className={`w-20 h-20 rounded-full mx-auto mb-3 bg-gradient-to-br from-primary to-primary/60 flex items-center justify-center text-white text-2xl font-heading font-black shadow-lg ${
                        isFirst ? "ring-4 ring-amber-300 ring-offset-2 mt-0 w-24 h-24" : "mt-6"
                      }`}>
                        {initials}
                      </div>
    ...
    <PageLayout>
      ...
      <section id="hero-section" className="relative min-h-[100vh] flex items-center">
        ...
        >
          ...
          <div className="w-7 h-12 rounded-full border-2 border-white/40 flex items-start justify-center p-1.5">
            <motion.div
              animate={{ y: [0, 16, 0] }}
              transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
