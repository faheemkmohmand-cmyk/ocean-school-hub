...
// ── Animation wrapper: supports Three.js r128, p5.js, or vanilla JS ──────────
// Three.js: detected by presence of THREE keyword
// p5.js: detected by setup()/draw() function signatures
...
function wrapP5Code(code: string): string {
  ...
  const isThree = /\bTHREE\b/.test(code) || /\bnew\s+THREE\./.test(code);
  ...
  const baseHead = [
    ...
    '<style>*{margin:0;padding:0;box-sizing:border-box}html,body{width:100%;height:100%;overflow:hidden;background:#0a0a1a}',
    'canvas{display:block;width:100%!important;height:100%!important;outline:none}',
    '#c{width:100%;height:100%;position:relative}</style>',
    '</head><body>',
  ].join('');
  ...
  if (isThree) {
    return [
      ...
      '  if(window.__renderer){window.__renderer.setSize(innerWidth,innerHeight);',
  ...
  if (isP5) {
    return [
      baseHead.replace('background:#0a0a1a','background:#f8f9ff'),
  ...
  return [
    ...
    '<script>window.addEventListener("load",function(){var container=document.getElementById("container");', safe, '});</script></body></html>',
  ].join('');
}
...
const P5Playground = ({
  ...
  const applyTemplate = (key: string, src: "three" | "p5" = "p5") => {
    const t = src === "three" ? THREE_TEMPLATES[key] : P5_TEMPLATES[key];
    if (!t) return;
    handleChange(t.code);
    setShowPreview(true);
    setPreviewKey(k => k + 1);
      ...
      <CardContent className="pt-5 space-y-4">
        ...
        <div className="bg-gradient-to-r from-violet-50 to-indigo-50 dark:from-violet-900/20 dark:to-indigo-900/20 border border-violet-200 dark:border-violet-700/30 rounded-2xl p-4">
          <p className="text-sm font-bold text-violet-700 dark:text-violet-300 mb-1 flex items-center gap-2">
            <Zap className="w-4 h-4" /> 3D Interactive Animation — Three.js + p5.js
          </p>
          <p className="text-xs text-violet-600 dark:text-violet-400 leading-relaxed">
            Paste <strong>Three.js</strong> code (uses <code className="bg-violet-100 dark:bg-violet-900/40 px-1 rounded">THREE</code> global, r128) <strong>or</strong> <strong>p5.js</strong> code (<code className="bg-violet-100 dark:bg-violet-900/40 px-1 rounded">setup()</code>/<code className="bg-violet-100 dark:bg-violet-900/40 px-1 rounded">draw()</code>).
            The system auto-detects which library to load. Students see a fully interactive 3D animation below the chapter.
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Label className="block text-sm font-semibold pt-1">p5.js Templates (2D)</Label>
          <div className="flex flex-wrap gap-2">
            {Object.entries(P5_TEMPLATES).map(([key, t]) => (
              <button key={"p-"+key} onClick={() => applyTemplate(key, "p5")}
          ...
          <div className={`grid gap-4 ${showPreview ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1"}`}>
            ...
            <div>
              <textarea
                ...
                spellCheck={false}
                placeholder={"// Paste Three.js or p5.js code here\n\n// THREE.JS: uses THREE global (r128)\n// const scene = new THREE.Scene();\n// ... your 3D scene ...\n\n// P5.JS: uses setup()/draw()\n// function setup() { createCanvas(windowWidth, windowHeight); }\n// function draw() { background(20); }"}
              ...
              <p className="text-[10px] text-muted-foreground mt-1.5 flex flex-wrap items-center gap-2">
                ...
                <span>•</span>
                <span>Three.js r128 + p5.js 1.9.4 — auto-detected</span>
                <span>•</span>
