          ...
          <CardContent className="pt-5 space-y-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-1.5">
                ✨ WYSIWYG editor — what you see is exactly what students will see.
              </p>
            </div>
          </CardContent>
        </Card>
...
const AdminNotesWithTests = () => {
  // For SRS panel we need the logged-in userId — read from Supabase session
  const [srsUserId, setSrsUserId] = useState<string | null>(null);

  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      if (data?.user?.id) setSrsUserId(data.user.id);
    });
  }, []);
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Manage notes content, MCQ tests and spaced repetition</p>
      </div>
      <Tabs defaultValue="notes" className="w-full">
        <TabsList className="w-full grid grid-cols-3 sm:inline-flex sm:w-auto">
          ...
          <TabsTrigger value="srs" className="gap-1.5 text-xs sm:text-sm">
        ...
        <TabsContent value="srs" className="mt-4">
          {srsUserId ? (
            <SRSReviewPanel userId={srsUserId} />
          ) : (
            <div className="space-y-3">
