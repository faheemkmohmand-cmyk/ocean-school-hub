...
// ── Graph Builder ─────────────────────────────────────────────────────────────
const GraphBuilder = ({ value, onChange }: { value: string; onChange: (v: string) => void }) => {
  const [mode, setMode] = useState<"equation"|"data"|"json">("equation");
  const [eq, setEq] = useState("");
  const [chartType, setChartType] = useState("line");
  const [title, setTitle] = useState("");
  const [rows, setRows] = useState([{ name: "", value: "", value2: "" }]);

  const buildJSON = () => {
    if (mode === "equation") {
      return JSON.stringify({ type: "line", equation: eq, title: title || `y = ${eq}`, xRange: [-10, 10] }, null, 2);
    }
    if (mode === "data") {
      const data = rows.filter(r => r.name && r.value).map(r => ({
        name: r.name,
        value: parseFloat(r.value) || 0,
        ...(r.value2 ? { value2: parseFloat(r.value2) || 0 } : {})
      }));
      return JSON.stringify({ type: chartType, title, data }, null, 2);
    }
    return value;
  };

  const apply = () => onChange(buildJSON());

  return (
    <div className="border border-border rounded-2xl overflow-hidden">
      <div className="flex border-b border-border">
        {(["equation","data","json"] as const).map(m => (
          <button key={m} onClick={() => setMode(m)}
            className={`flex-1 py-2.5 text-xs font-bold capitalize transition-colors ${mode === m ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:bg-secondary/70"}`}>
            {m === "equation" ? "📈 Equation" : m === "data" ? "📊 Data" : "⚙️ JSON"}
          </button>
        ))}
      </div>
      <div className="p-4 space-y-3">
        {mode === "equation" && (
          <>
            <div><Label className="text-xs">Title</Label><Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Graph title..." className="mt-1" /></div>
            <div>
              <Label className="text-xs">Equation (y = ...)</Label>
              <Input value={eq} onChange={e => setEq(e.target.value)} placeholder="x^2 or sin(x) or 2*x+1" className="mt-1 font-mono" />
              <p className="text-xs text-muted-foreground mt-1">Supports: x^2, sin(x), cos(x), tan(x), sqrt(x), log(x), pi</p>
            </div>
          </>
        )}
        {mode === "data" && (
          <>
            <div className="grid grid-cols-2 gap-2">
              <div><Label className="text-xs">Chart Type</Label>
                <select value={chartType} onChange={e => setChartType(e.target.value)}
                  ...
                  <option value="area">Area</option><option value="pie">Pie</option>
                </select>
              </div>
              <div><Label className="text-xs">Title</Label><Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Title..." className="mt-1" /></div>
            </div>
            <div>
              <Label className="text-xs mb-2 block">Data Points</Label>
              {rows.map((r, i) => (
                <div key={i} className="flex gap-2 mb-2">
                  <Input value={r.name} onChange={e => { const n=[...rows]; n[i]={...n[i],name:e.target.value}; setRows(n); }} placeholder="Label" className="flex-1" />
                  <Input value={r.value} onChange={e => { const n=[...rows]; n[i]={...n[i],value:e.target.value}; setRows(n); }} placeholder="Value" type="number" className="w-20" />
                  <Input value={r.value2} onChange={e => { const n=[...rows]; n[i]={...n[i],value2:e.target.value}; setRows(n); }} placeholder="Val2" type="number" className="w-20" />
                  <button onClick={() => setRows(rows.filter((_,j)=>j!==i))} className="p-2 text-destructive hover:bg-destructive/10 rounded-lg shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              ))}
              <button onClick={() => setRows([...rows,{name:"",value:"",value2:""}])}
                className="text-xs text-primary flex items-center gap-1 mt-1"><Plus className="w-3.5 h-3.5" /> Add Row</button>
            </div>
          </>
        )}
        {mode === "json" && (
          <textarea value={value} onChange={e => onChange(e.target.value)} rows={6}
            placeholder='{"type":"line","equation":"x^2","title":"Parabola"}'
        ...
        {mode !== "json" && (
          <Button onClick={apply} size="sm" className="w-full gap-2"><Zap className="w-3.5 h-3.5" /> Apply Graph</Button>
        )}
      </div>
    </div>
  );
