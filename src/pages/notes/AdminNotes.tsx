...
function wrapP5Code(code: string): string {
  ...
  // Escape any </script> tags inside user code
  const safe = code.replace(/<\/script>/gi, "<\\/script>");

  const isP5 = /\bfunction\s+setup\s*\(/.test(code) || /\bfunction\s+draw\s*\(/.test(code);

  if (isP5) {
    // p5 GLOBAL MODE: load p5 lib first, then sketch in a separate <script>.
    // p5 auto-finds window.setup and window.draw. Do NOT use IIFE or instance
    // mode — those trap functions in local scope and break the preview.
    const interceptor = [
