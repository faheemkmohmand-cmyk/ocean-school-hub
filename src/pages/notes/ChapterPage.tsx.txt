...
function buildSrcDoc(code: string): string {
  ...
  if (trimmed.startsWith("<!doctype") || trimmed.startsWith("<html")) {
    // Already full HTML — inject mobile viewport meta if missing
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  html,body{width:100%;height:100%;overflow:hidden;background:#f8f9ff;touch-action:none}
  canvas{display:block;touch-action:none}
  #c{width:100%!important;height:100%!important}
</style>
</head>
<body>
<div id="c"></div>
...
<script>
(function() {
  // Execute the sketch code to register setup/draw in local scope
  ${code}
  // Build p5 instance using whatever setup/draw are now defined
  new p5(function(p) {
    if (typeof setup === 'function') {
      p.setup = function() {
        const cnv = p.createCanvas(window.innerWidth, window.innerHeight);
        cnv.parent('c');
        try { setup.call(p); } catch(e) {}
        // Override canvas size after user setup
    ...
    if (typeof draw === 'function') {
      p.draw = function() { try { draw.call(p); } catch(e) {} };
    }
    // Wire mouse/touch/key handlers if defined in sketch
    ['mousePressed','mouseReleased','mouseMoved','mouseDragged',
     'keyPressed','keyReleased','touchStarted','touchMoved','touchEnded'].forEach(fn => {
      if (typeof window[fn] === 'function') p[fn] = window[fn].bind(p);
    });
    p.windowResized = function() {
      p.resizeCanvas(window.innerWidth, window.innerHeight);
      if (typeof windowResized === 'function') try { windowResized.call(p); } catch(e) {}
    };
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  canvas{touch-action:none;max-width:100%}
</style>
</head>
<body>
${code}
