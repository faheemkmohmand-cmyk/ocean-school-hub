...
function buildSrcDoc(code: string): string {
  ...
  // Detect library type
  const isThree = /\bTHREE\b/.test(code) || /\bnew\s+THREE\./.test(code) || /\bimport.*three/.test(code.toLowerCase());
  ...
  const commonHead = [
    ...
    'canvas{display:block;touch-action:none;width:100%!important;height:100%!important;outline:none}',
    '#three-container{width:100%;height:100%;position:relative;overflow:hidden}',
    '</style>',
    '</head><body>',
  ].join('');

  // ── THREE.JS — load r128 from CDN, inject resize helper, run sketch ──────────
  if (isThree) {
    return [
      commonHead,
      ...
      '<script>',
      // Resize helper injected automatically so admin doesn't need to write it
      '(function(){',
      '  window.__threeContainer = document.getElementById("three-container");',
      '  window.__threeW = function(){ return window.__threeContainer.clientWidth||window.innerWidth; };',
      '  window.__threeH = function(){ return window.__threeContainer.clientHeight||window.innerHeight; };',
      '  // Auto-resize: call user's onWindowResize if defined, else resize known renderer',
      '  window.addEventListener("resize",function(){',
      '    if(typeof onWindowResize==="function"){ onWindowResize(); return; }',
      '    if(window.__renderer){',
      '      window.__renderer.setSize(window.__threeW(),window.__threeH());',
      '      if(window.__camera){',
      '        window.__camera.aspect=window.__threeW()/window.__threeH();',
  ...
  // ── P5.JS — load 1.9.4 from CDN, run in global mode ─────────────────────────
  if (isP5) {
    return [
      commonHead.replace('background:#0a0a1a','background:#f8f9ff'),
