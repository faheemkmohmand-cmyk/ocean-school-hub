...
function buildSrcDoc(code: string): string {
  ...
  // ── Already full HTML — just ensure mobile viewport exists ──────────────────
  ...
  // ── p5.js sketch (has setup() or draw()) ────────────────────────────────────
  // CRITICAL: We run p5 in GLOBAL mode by loading the sketch as a plain <script>
  // tag AFTER p5.js loads. In global mode p5 automatically picks up setup/draw
  // defined on the window. No IIFE, no instance wrapping — that's what breaks it.
  const isP5 = /\bfunction\s+setup\s*\(/.test(code) || /\bfunction\s+draw\s*\(/.test(code);
  if (isP5) {
    // Escape </script> inside the code so it doesn't break the HTML parser
...
<body>
<!-- p5 global mode: load library first, then sketch. p5 auto-calls setup/draw. -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/p5.js/1.9.4/p5.min.js"></script>
<script>
// Patch: make createCanvas fill the iframe viewport automatically
const _origSetup = window.setup;
window.setup = function() {
  if (typeof _origSetup === 'function') {
    _origSetup();
    // After user setup runs, resize to fill viewport in case they used a fixed size
    if (typeof resizeCanvas === 'function') resizeCanvas(windowWidth, windowHeight);
  } else {
    createCanvas(windowWidth, windowHeight);
  }
};
const _origWR = window.windowResized;
window.windowResized = function() {
  resizeCanvas(windowWidth, windowHeight);
  if (typeof _origWR === 'function') _origWR();
};
</script>
<script>
\${safeCode}
  ...
  // ── Plain JS / vanilla Canvas fallback ──────────────────────────────────────
  const safeCode2 = code.replace(/<\/script>/gi, '<\\/script>');
  return \`<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
<body>
<script>\${safeCode2}</script>
</body>
</html>\`;
}

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// InteractiveIframe — mobile-first p5.js / HTML animation viewer for students
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
const InteractiveIframe = ({ code, subjectColor }: { code: string; subjectColor: string }) => {
  ...
  // Height: on mobile use aspect ratio 4:3 (portrait friendly), on desktop 480px
  const iframeStyle = {
    border: "none",
    display: "block",
    width: "100%",
    ...
    <>
        ...
        <div className="fixed inset-0 z-[9999] bg-black flex flex-col" style={{ touchAction: "none" }}>
          <div className="flex items-center justify-between px-4 py-2 bg-gray-900 shrink-0">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setIframeKey(k => k + 1)}
                className="text-xs text-gray-300 hover:text-white px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 flex items-center gap-1.5 active:scale-95">
                <RotateCcw className="w-3.5 h-3.5" /> Restart
              </button>
              <button onClick={() => setFullscreen(false)}
                className="text-xs text-gray-300 hover:text-white px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 active:scale-95">
                ✕ Close
              </button>
            </div>
          </div>
          <div className="flex-1 relative overflow-hidden">
            <iframe
              ...
              style={{ ...iframeStyle, position: "absolute", inset: 0, height: "100%" }}
      ...
      <div className="mt-8" ref={containerRef}>
        ...
        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setIframeKey(k => k + 1)}
              className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground px-3 py-1.5 rounded-xl border border-border hover:bg-secondary transition-colors active:scale-95">
              <RotateCcw className="w-3 h-3" /> Restart
            </button>
            <button
              onClick={() => setFullscreen(true)}
              className="flex items-center gap-1.5 text-xs font-semibold text-white px-3 py-1.5 rounded-xl hover:opacity-90 transition-opacity active:scale-95"
        ...
        {/* Iframe container — intrinsic 4:3 ratio on mobile, 480px on sm+ */}
        <div
          className="relative rounded-2xl overflow-hidden border-2 shadow-lg w-full"
          style={{ borderColor: `${subjectColor}40` }}
        >
          {/* Invisible spacer: 4:3 on mobile (<640px), 480px on sm+ */}
          <div className="sm:hidden" style={{ paddingBottom: "75%" }} />
          <div className="hidden sm:block" style={{ height: "480px" }} />
          {/* Iframe fills the spacer absolutely */}
        ...
        <p className="text-[10px] text-muted-foreground mt-1.5 text-center">
          🔒 Safe sandbox · Tap Fullscreen for best experience on mobile
        </p>
      </div>
    </>
  );
