...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Animation code wrapper — supports Three.js, p5.js, vanilla Canvas, or full HTML
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function buildSrcDoc(code: string): string {
  const trimmed = code.trim().toLowerCase();
  const isThree = code.includes("THREE.") || code.includes("new THREE") || code.toLowerCase().includes("import") && code.toLowerCase().includes("three");
  ...
  const ensureThreeContainer = (html: string) => {
    let output = html;
    if (!/id=["']c["']/i.test(output)) {
      output = output.replace(
        /<head>/i,
        '<head><style>*{box-sizing:border-box}html,body{width:100%;height:100%;margin:0;overflow:hidden;background:#0a0a1a}body{position:relative}#c{position:relative;width:100%;height:100%;overflow:hidden;isolation:isolate}canvas{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;display:block!important;outline:none;touch-action:none}</style>'
      );
      output = output.replace(/<body([^>]*)>/i, '<body$1><div id="c"></div>');
    }
    if (!output.includes('window.__container')) {
      output = output.replace(/<body([^>]*)>/i, '<body$1><script>window.__container=document.getElementById("c")||document.body;window.__mount=window.__container;</script>');
    }
    return output;
  };

  // Already full HTML — pass through, just ensure viewport meta exists
  if (trimmed.startsWith("<!doctype") || trimmed.startsWith("<html")) {
    let html = ensureViewport(code);
    if (isThree) html = ensureThreeContainer(html);
    return html;
  }

  // Escape </script> to prevent HTML parser breaking
  const safe = code.replace(/<\/script>/gi, "<\\/script>");

  const buildHead = (background: string, foreground: string) => [
    ...
    `body{position:relative;background:${background};color:${foreground};font-family:system-ui,sans-serif}`,
    '#c{position:relative;width:100%;height:100%;overflow:hidden;isolation:isolate}',
    'canvas{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;display:block!important;outline:none;touch-action:none}',
    '</style>',
    '</head><body><div id="c"></div>',
  ].join('');

  if (isThree) {
    // Inject resize helper: auto-resizes renderer when user defines window.__renderer & window.__camera
    const resizeHelper = [
      'window.__container=document.getElementById("c")||document.body;',
      'window.__mount=window.__container;',
      'window.addEventListener("resize",function(){',
      '  if(typeof onWindowResize==="function"){onWindowResize();return;}',
      '  if(window.__renderer){',
      '    window.__renderer.setSize(innerWidth,innerHeight);',
      '    if(window.__camera){window.__camera.aspect=innerWidth/innerHeight;window.__camera.updateProjectionMatrix();}',
      '  }',
      '});',
    ].join('');

    return [
      buildHead('#0a0a1a', '#ffffff'),
  ...
  // Vanilla JS / Canvas fallback (no p5, no Three.js)
  return [
    buildHead('#f8f9ff', '#111827'),
    '<script>',
    'window.__container=document.getElementById("c")||document.body;',
    'window.__mount=window.__container;',
    safe,
