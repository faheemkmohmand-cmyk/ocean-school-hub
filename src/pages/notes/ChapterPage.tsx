...
function buildSrcDoc(code: string): string {
  ...
  if (isP5) {
    ...
    return [
      commonHead,
      // Primary CDN
      '<script src="https://unpkg.com/p5@1.9.4/lib/p5.min.js"></scr'+'ipt>',
      '<script>',
...
const InteractiveIframe = ({ code, subjectColor }: { code: string; subjectColor: string }) => {
  ...
  // Use blob URL instead of srcDoc — srcDoc iframes cannot load external CDN
  // scripts (like p5.js) due to CSP restrictions. Blob URLs create a separate
  // origin that CAN fetch external resources, so p5.js animations render properly.
  useEffect(() => {
