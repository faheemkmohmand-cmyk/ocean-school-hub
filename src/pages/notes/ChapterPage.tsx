...
function buildSrcDoc(code: string): string {
  ...
  // Already full HTML — ensure mobile viewport
  ...
  // Escape any </script> inside user code so it doesn't break the HTML parser
  ...
  if (isP5) {
    // ── p5 GLOBAL MODE ────────────────────────────────────────────────────────
    // Load p5.js first, then the sketch as a plain <script>.
    // p5 global mode auto-calls window.setup and window.draw — no IIFE, no
    // instance wrapping (those trap setup/draw in local scope and break it).
    const interceptor = [
      "const _uSetup = window.setup;",
      "window.setup = function() {",
      "  if (typeof _uSetup === 'function') { _uSetup(); }",
      "  if (typeof resizeCanvas === 'function') resizeCanvas(windowWidth, windowHeight);",
      "};",
      "const _uWR = window.windowResized;",
      "window.windowResized = function() {",
      "  resizeCanvas(windowWidth, windowHeight);",
      "  if (typeof _uWR === 'function') _uWR();",
      "};",
    ].join("\n");
