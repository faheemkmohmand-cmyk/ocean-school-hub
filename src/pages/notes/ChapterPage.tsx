...
const AudioPlayer = ({ content, onClose }: { content: string; onClose: () => void }) => {
  ...
  const speakChunk = useCallback((idx: number) => {
    ...
    if (!text) { speakChunk(idx + 1); return; }
    ...
    utt.onerror = (e) => {
      // "interrupted" is normal when we cancel to go to next chunk
      if (e.error === "interrupted" || e.error === "canceled") return;
      // Retry once on other errors
      setTimeout(() => { if (activeRef.current) speakChunk(idx); }, 300);
    };

    window.speechSynthesis.speak(utt);
  }, []);

  // keepAlive: Chrome silently stops synthesis after ~15s
  // Solution: pause/resume every 10s to reset the timer
  const startKeepAlive = useCallback(() => {
    clearInterval(keepAliveRef.current);
    keepAliveRef.current = setInterval(() => {
      if (activeRef.current && window.speechSynthesis.speaking && !window.speechSynthesis.paused) {
        window.speechSynthesis.pause();
        setTimeout(() => { if (activeRef.current) window.speechSynthesis.resume(); }, 50);
      }
    }, 10000);
  }, []);
...
const ChapterPage = () => {
  ...
  useEffect(() => {
    ...
    try {
      const wrapped = chapter.animation_code.replace(/setInterval\s*\(/g, "__ti(").replace(/canvas\.width\s*=\s*(\d+)/g, "canvas.width=Math.min($1,container.offsetWidth||$1)");
      new Function("container","__ti",wrapped)(container,(cb:()=>void,ms:number)=>{const id=setInterval(cb,ms);ids.push(id);return id;});
    } catch(e:any) {
      container.innerHTML=`<div style="padding:20px;color:#ef4444;font-family:monospace;font-size:13px;background:#fef2f2;border-radius:12px">⚠️ Error: ${e.message}</div>`;
    }
    return () => { ids.forEach(id=>clearInterval(id)); container.innerHTML=""; };
      ...
      <div className="max-w-5xl mx-auto px-3 md:px-4 py-6 md:py-8 flex gap-6 md:gap-8">
        ...
        <main className="flex-1 min-w-0">
          ...
          {chapter.animation_code && !liteMode && (
            <div className="mt-8">
              <h3 className="text-lg font-bold text-foreground mb-3 flex items-center gap-2"><Zap className="w-5 h-5 text-violet-500" /> Interactive Demo</h3>
              <div ref={animRef} className="bg-white dark:bg-zinc-900 border border-border rounded-2xl" style={{minHeight:"320px",width:"100%",overflowX:"auto",position:"relative"}} />
              <button onClick={()=>{
                if(!animRef.current||!chapter.animation_code) return;
                const c=animRef.current; c.innerHTML="";
                const ids:ReturnType<typeof setInterval>[]=[];
                try { const w=chapter.animation_code.replace(/setInterval\s*\(/g,"__ti(");
                  new Function("container","__ti",w)(c,(cb:()=>void,ms:number)=>{const id=setInterval(cb,ms);ids.push(id);return id;}); }
                catch{}
