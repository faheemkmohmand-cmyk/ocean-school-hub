...
function wrapP5Code(code: string): string {
  // Detect if it's already full HTML
  const trimmed = code.trim().toLowerCase();
  if (trimmed.startsWith("<!doctype") || trimmed.startsWith("<html")) {
    return code;
  }
  // Detect if it's a p5 sketch (uses setup/draw functions)
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  #p5-container { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; }
</style>
</head>
<body>
...
<script>
// Auto-resize p5 canvas to fit container
const _origSetup = window.setup;
new p5(function(p) {
  // Inject all sketch functions into this p5 instance
  ${code.replace(/\b(setup|draw|mousePressed|mouseMoved|mouseDragged|mouseReleased|keyPressed|keyReleased|touchStarted|touchMoved|touchEnded|windowResized)\b\s*(?=\()/g,
    (match) => `p.${match}`
  )}
  ...
  p.setup = function() {
    ...
    if (typeof setup === 'function') setup.call(p);
  };

  p.draw = function() {
    if (typeof draw === 'function') draw.call(p);
  };
  ...
  // Plain JS/Canvas — wrap in basic HTML
  return `<!DOCTYPE html>
<html>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
...
const P5Playground = ({
  ...
  const srcDoc = code.trim() ? wrapP5Code(code) : "";
