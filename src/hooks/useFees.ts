...
export function useStudentVouchers(classLevel?: string | null, studentId?: string | null) {
  return useQuery<FeeVoucher[]>({
    ...
    queryFn: async () => {
      ...
      // Fetch all vouchers for the student's class.
      // RLS policies (fv_student_own + fv_teacher_read) control row visibility.
      // The UI filters "my vouchers" by student_id === profile.id on the client side.
      let query = supabase
        .from("fee_vouchers")
        .select("*, students(full_name, roll_number, father_name, contact_number, photo_url)")
        .eq("class", classLevel)
