...
export function useStudentVouchers(studentId?: string) {
  return useQuery<FeeVoucher[]>({
    queryKey: ["student-vouchers", studentId],
    queryFn: async () => {
      if (!studentId) return [];
      const { data, error } = await supabase
        .from("fee_vouchers")
    ...
    enabled: !!studentId,
    staleTime: 15 * 1000, // Reduced from 2min → 15sec for faster updates
    refetchInterval: 30 * 1000, // Auto-refetch every 30s so new vouchers appear immediately
  });
}
...
export function useAllVouchersForStudents(classLevel?: string) {
  return useQuery<FeeVoucher[]>({
