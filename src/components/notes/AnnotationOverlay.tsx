...
const CreateAnnotationPopup = ({
  ...
  return (
    <motion.div
      ...
      className="fixed z-50 w-80 max-w-[calc(100vw-2rem)] bg-card border border-border rounded-2xl shadow-2xl overflow-hidden"
      style={{ left: Math.min(position.x, window.innerWidth - 340), top: Math.min(position.y, window.innerHeight - 350) }}
    >
...
const AnnotationOverlay = ({ chapterId, contentRef }: AnnotationOverlayProps) => {
  const { user } = useAuth();
  const { data: annotations = [] } = useAnnotations(chapterId, user?.id);
  ...
  return (
    <>
      {/* Annotation toggle button — mobile-friendly floating button */}
      <button
        ...
        {showAnnotationList && (
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-border">
              ...
              <button onClick={() => setShowAnnotationList(false)} className="p-1.5 hover:bg-secondary rounded-lg">
                <X className="w-4 h-4" />
              </button>
            </div>
