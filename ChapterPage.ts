...
const AudioPlayer = ({ content, onClose }: { content: string; onClose: () => void }) => {
  ...
  // Keep speedRef in sync so play() closure always has latest value
  ...
  const play    = () => { setStatus("playing"); startSpeech(speedRef.current); };
  ...
  const changeSpeed = (s: number) => {
    setSpeed(s);
    speedRef.current = s;
    if (status !== "idle") { setStatus("playing"); startSpeech(s); }
  };
  ...
  return (
    <div className="fixed bottom-20 right-4 z-40 bg-card border border-border rounded-2xl shadow-xl p-4 w-72">
      ...
      <div className="flex gap-2 mb-3">
        ...
        <button onClick={stop} disabled={status === "idle"} className="p-2 rounded-xl border border-border hover:bg-secondary disabled:opacity-40">
      ...
      {status === "idle" && (
        <p className="text-[10px] text-muted-foreground text-center mt-2">Press Play to start reading</p>
      )}
      {status === "playing" && (
        <p className="text-[10px] text-green-500 text-center mt-2 flex items-center justify-center gap-1">
          <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse inline-block" /> Reading aloud...
        </p>
      )}
      {status === "paused" && (
        <p className="text-[10px] text-amber-500 text-center mt-2">Paused — press Resume to continue</p>
      )}
    </div>
  );
};
...
const ChapterPage = () => {
  ...
  // Fixed animation runner with cleanup + mobile-safe sizing
  useEffect(() => {
    ...
    try {
      // Patch code: track setInterval, also patch canvas width to container width
      const wrappedCode = chapter.animation_code
        .replace(/setInterval\s*\(/g, "__trackedInterval(")
        // Make canvas auto-size to container on mobile
        .replace(
